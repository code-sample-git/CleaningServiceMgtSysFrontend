      ...
      <nav className="navbar">
        <div className="navbar-content">
          ...
          <div className="navbar-menu">
            <span className="navbar-link">Hello, {user?.firstName || "User"}</span>
            ...
            {inventory.map((item) => (
              <div key={item.id} className="card">
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  <button onClick={() => handleUpdateItem(item.id, { quantity: item.quantity + 1 })}>+</button>
                  <button onClick={() => handleUpdateItem(item.id, { quantity: Math.max(0, item.quantity - 1) })}>-</button>
                  <button
